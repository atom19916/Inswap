        ...
        with gr.Row(variant='panel'):
            ...
            with gr.Column(scale=2):
                ...
                maskimage = gr.ImageEditor(
                    ...
                    interactive=True,
                    visible=False,
                    layers=[create_blank_image(512, 512)]
                )
                with gr.Row(variant='panel'):
                    fake_preview = gr.Checkbox(label="Face swap frames", value=False)
                    bt_refresh_preview = gr.Button("🔄 Refresh", variant='secondary', size='sm')
                    bt_use_face_from_preview = gr.Button("Use Face from this Frame", variant='primary', size='sm')
                with gr.Row():
                    preview_frame_num = gr.Slider(minimum=0, maximum=1, value=0, label="Frame Number", info='0:00:00', step=1.0, interactive=True)
                with gr.Row():
                    text_frame_clip = gr.Markdown('Processing frame range [0 - 0]')
                    set_frame_start = gr.Button("⬅ Set as Start", size='sm')
                    set_frame_end = gr.Button("➡ Set as End", size='sm')
...
def on_toggle_masking(previewimage, mask):
    ...
    if manual_masking:
        ...
        return (
            gr.Image(visible=False),
            gr.ImageEditor(value={"background": previewimage, "layers": layers, "composite": None}, visible=True)
        )
    return gr.Image(visible=True), gr.ImageEditor(visible=False)
...
def start_swap(swap_model, output_method, enhancer, detection, keep_frames, wait_after_extraction, skip_audio, face_distance, blend_ratio,
    ...
    if not util.is_installed("ffmpeg"):
        gr.Warning("ffmpeg is not installed! No video processing possible.")
...
def on_destfiles_changed(destfiles):
    ...
    if destfiles is None or len(destfiles) < 1:
        list_files_process.clear()
        return gr.Slider(minimum=0, value=0, maximum=1, info='0:00:00'), ''
    ...
    safe_max = max(total_frames, 2)
    if total_frames > 1:
        return gr.Slider(minimum=0, value=1, maximum=safe_max, info='0:00:00'), gen_processing_text(list_files_process[idx].startframe, list_files_process[idx].endframe)
    return gr.Slider(minimum=0, value=1, maximum=safe_max, info='0:00:00'), ''
...
def on_destfiles_selected(evt: gr.SelectData):
    ...
    safe_max = max(total_frames, 2)
    if total_frames > 1:
        return gr.Slider(minimum=0, value=list_files_process[idx].startframe, maximum=safe_max, info='0:00:00'), gen_processing_text(list_files_process[idx].startframe, list_files_process[idx].endframe), fps
    return gr.Slider(minimum=0, value=1, maximum=safe_max, info='0:00:00'), gen_processing_text(0, 0), fps
